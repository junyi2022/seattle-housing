...
# Add-in Data
## social
crime <- read.csv(file.path("C:/Users/25077/Desktop/PPA_Midterm/data/SPD_Crime_Data.csv"))
...
Seattle_final <- st_drop_geometry(Seattle.sf_tracts2) %>%
  ...
  select(Age, price, bedrooms, bathrooms, sqft_living, floors, waterfront, view, condition, grade, crime_count, hosp_nn1, school_nn1, parks_count, cross_count, streetcar.station, canopy, TrafficDis) %>% 
...
## comparison model
reg2 <- lm(price ~ Age + bedrooms +  
                 sqft_living + floors + waterfront + I(view^2) + condition + grade + crime_count +   + hosp_nn1 + I(school_nn1^2) +
                 parks_count + I(cross_count^2) +
                 streetcar.station + canopy + TrafficDis, 
           data = Seattle_final)
