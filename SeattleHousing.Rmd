...
```{r crime_corr, fig.height=6, fig.width=10}
...
ggplot(Seattle_Crime_long, aes(Value, price)) +   # Set up the plot
  geom_point(size = .5) +                       # Add points for each data point
  geom_smooth(method = "lm", se = FALSE, colour = "#FA7800") +  # Add linear regression lines
  facet_wrap(~Variable, nrow = 2, scales = "free") +  # Create separate plots for each variable
  labs(title = "Price as a function of continuous variables") +  # Set plot title
  theme_minimal()  # Apply a minimal theme to the plot
...
# coefficients_table2 <- as.data.frame(summary_training$coefficients)
# 
# coefficients_table2$significance <- ifelse(coefficients_table2$`Pr(>|t|)` < 0.001, '***',
#                                     ifelse(coefficients_table2$`Pr(>|t|)` < 0.01, '**',
#                                       ifelse(coefficients_table2$`Pr(>|t|)` < 0.05, '*',
#                                         ifelse(coefficients_table2$`Pr(>|t|)` < 0.1, '.', ''))))
# 
# coefficients_table2$p_value <- paste0(round(coefficients_table2$`Pr(>|t|)`, digits = 3), coefficients_table2$significance)
# coefficients_table2$'t value' <- round(coefficients_table2$'t value', digits = 2)
# coefficients_table2$'Std. Error' <- round(coefficients_table2$'Std. Error', digits = 2)
# coefficients_table2$Estimate <- round(coefficients_table2$Estimate, digits = 2)
# 
# coefficients_table2 %>%
#   select(-significance, -`Pr(>|t|)`) %>%
#   kable(align = "r") %>%
