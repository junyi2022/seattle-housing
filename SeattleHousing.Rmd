...
## pull
table_Seattle <- as.data.frame(Seattle.sf_tracts2)
table_Seattle <- Seattle.sf_tracts2 %>% select(-geometry)


#category of the data
internal_character <- table_Seattle[, c("bedrooms", "bathrooms", "sqft_living","sqft_lot", "floors", "condition", "grade")] %>% summary()
Structure_character <- table_Seattle[, c("condition", "grade", "Age", "yr_renovated")] %>% summary()
amenities <- table_Seattle[, c("parksCount", "schoolCount", "hospitalCount", "crosswalkCount", "canopyPercent", "distance", "hosp_nn1", "school_nn1")] %>% summary()
spatial_character <- table_Seattle[, c( "waterfront", "view")] %>% summary()
safety <- table_Seattle[, c( "crime_nn1","crime_nn2","crime_nn3","crime_nn4","crime_nn5", "crimeCount")] %>% summary()
Demographics <- table_Seattle[, c( "TotalPop", "MedIncome", "pctWhite")] %>% summary()

# Combine the summaries into one data frame
summary_data <- cbind(internal_character, Structure_character, amenities, 
                      spatial_character, safety, Demographics)

summary_data <- summary(table_Seattle[, "bedrooms"])

# Capture summary output as text
summary_text <- capture.output(summary_data)

# Process summary text to extract values into a data frame
summary_df <- as.data.frame(matrix(unlist(strsplit(summary_text, "\\s+")), ncol = 6, byrow = TRUE))
colnames(summary_df) <- c("Min", "Q1", "Median", "Mean", "Q3", "Max")

# Remove first row (labels)
summary_df <- summary_df[2, ]

# Convert values to numeric (except for 'Statistic' column)
summary_df[, -1] <- sapply(summary_df[, -1], as.numeric)



stargazer(summary_data, Structure_character, amenities, spatial_character, safety, Demographics, 
          type = "text", 
...
Seattle_final <- Seattle_final_0 %>%
  select(Age, price, bedrooms, bathrooms, sqft_living, waterfront, view, condition, grade,
