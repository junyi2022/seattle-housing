...
internal_character <- table_Seattle[, c("bedrooms", "bathrooms", "sqft_living","sqft_lot", "floors", "condition", "grade")]
Structure_character <- table_Seattle[, c("condition", "grade", "Age", "yr_renovated")]
amenities <- table_Seattle[, c("parksCount", "schoolCount", "hospitalCount", "crosswalkCount", "canopyPercent", "distance", "hosp_nn1", "school_nn1")] 
spatial_character <- table_Seattle[, c( "waterfront", "view")]
safety <- table_Seattle[, c( "crime_nn1","crime_nn2","crime_nn3","crime_nn4","crime_nn5", "crimeCount")]
Demographics <- table_Seattle[, c( "TotalPop", "MedIncome", "pctWhite")]


stargazer(internal_character, Structure_character, amenities, spatial_character, safety, Demographics, 
          type = "text", 
...
ggplot() +
  ...
  geom_sf(data = streetcar, size = .5, color = "black") +  
...
ggplot() +
  geom_sf(data = tracts, fill = "lightgrey", col = "white") +
  geom_sf(data = Hospital, size = .5, color = "red") +
  labs(title = "Hospital") +
  theme_void()

## MedIncome
Demo_income <- st_join(Demo, Seattle.sf_tracts2, left = TRUE)
